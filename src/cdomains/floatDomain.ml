...
module FloatDomTupleImplFixedRoundingMode = struct
  ...
  *)
  ...
  let to_int = F2.to_int
  let cast_to = failwith "todo: we need our own - or at least we need the same code here, as we *at least* need to use the starting/ending from here!"
  let of_const = failwith "todo: we need our own I think"
  let of_interval = failwith "todo: we need our own I think"
  let of_string = failwith "todo: we need our own"
  let of_int = failwith "todo: we need our own"
  let top_of = F2.top_of
  let bot_of = F2.bot_of
  let ending = failwith "todo: we need our own"
  let starting = failwith "todo: we need our own"
  let minimal = F2.minimal
  let maximal = F2.maximal
  let is_exact = F2.is_exact
  let precision = F2.precision
end