...
struct
  ...
  and eval_rv_base (a: Q.ask) (gs:glob_fun) (st: store) (exp:exp): value =
    ...
    let r =
      ...
      (* only allow long doubles that are immediatly cast to a representable floating point type *)
      | CastE (TFloat ((FFloat | FDouble as fkind), _), Const (CReal (num, FLongDouble, _))) -> `Float (FD.of_const fkind num)
      (* String literals *)
