...
struct
  ...
  and eval_rv_base (a: Q.ask) (gs:glob_fun) (st: store) (exp:exp): value =
    ...
    let r =
      ...
      | Const (CReal (num, (FLongDouble), Some str)) when FloatOps.CDouble.atof Nearest str = FloatOps.CDouble.smallest -> `Float (FD.of_const FDouble num)
      | Const (CReal (num, (FLongDouble), Some str)) when FloatOps.CDouble.atof Nearest str = FloatOps.CDouble.upper_bound -> `Float (FD.of_const FDouble num)
      (* String literals *)
